...
template sudoku() {
    ...
    // create rangeproof components for checking the ranges of inputs
    
    component rp_puzzle[9][9];
    component rp_solution[9][9];
    
    
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            rp_puzzle[i][j] = RangeProof(32);
            rp_puzzle[i][j].in <== puzzle[i][j];
            rp_puzzle[i][j].range[0] <== 0;
            rp_puzzle[i][j].range[1] <== 9;

            rp_solution[i][j] = RangeProof(32);
            rp_solution[i][j].in <== puzzle[i][j];
            rp_solution[i][j].range[0] <== 0;
            rp_solution[i][j].range[1] <== 9;

            assert(rp_puzzle[i][j].out == 1);
            assert(rp_solution[i][j].out == 1);
